...
class MasterAccountController extends Controller
{
    ...
    public function allAccountmasterStatement(Request $request)
    {
        ...
        if($request->from_date){
            $invoiceCollection = Expense::join('payment_accounts','expenses.payment_account_id','payment_accounts.utilize_div_id')->join('account_categories','account_categories.id','expenses.account_category_id')->where('is_paid',1)->where('payment_accounts.type','division')->select('payment_accounts.name as div_name','payment_accounts.name as nick_name','account_categories.name as cat_name','expenses.utilize_div_id as divid','expenses.*')->whereBetween('expenses.created_at', [$request->from_date . ' ' . '00:00:00', $request->to_date ? $request->to_date . ' ' . '23:59:59' : now()])->get();
