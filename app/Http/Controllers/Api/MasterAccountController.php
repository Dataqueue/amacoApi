...
class MasterAccountController extends Controller
{
    ...
    public function masterStatement(Request $request)
    {
        ...
        $divEopenbalance=Expense::where('is_paid',1)->where('div_id',$div->id)->whereDate('created_at','<=' ,'2021-01-01 07:53:12')->sum('amount');
        $divRopenbalance=Receipt::where('div_id',$div->id)->whereDate('created_at','<=' ,'2021-01-01 07:53:12')->sum('paid_amount');
    ...
    public function allAccountmasterStatement(Request $request)
    {
        ...
        if($request->from_date){
            ...
            $divEopenbalance=Expense::where('is_paid',1)->whereDate('created_at','<=' ,date_format(strtotime($request->from_date),'Y-m-d H:i:s'))->sum('amount');
        }else{
        ...
        if($request->from_date){
            ...
            $divRopenbalance=Receipt::whereDate('created_at','<=' ,date_format(strtotime($request->from_date),'Y-m-d H:i:s'))->sum('paid_amount');
        }else{
