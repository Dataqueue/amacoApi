...
class MasterAccountController extends Controller
{
    ...
    public function allAccountmasterStatement(Request $request)
    {
        ...
        if($request->from_date){
            ...
            $divEopenbalance=Expense::where('created_at', '<=', $request->from_date. ' ' . '00:00:00')->sum(replace('amount', ',', ''));
        }else{
        ...
        if($request->from_date){
            ...
            $divRopenbalance=Receipt::where('created_at', '<=',$request->from_date. ' ' . '00:00:00')->sum(replace('paid_amount',',',''));
        }else{
