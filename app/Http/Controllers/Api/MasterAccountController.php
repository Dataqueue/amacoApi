...
class MasterAccountController extends Controller
{
    ...
    public function allAccountmasterStatement(Request $request)
    {
        ...
        if($request->from_date){
            ...
            $divEopenbalance=Expense::where('is_paid',1)->whereBetween('created_at','<=',$request->from_date)->sum('amount');
        }else{
        ...
        if($request->from_date){
            ...
            $divRopenbalance=Receipt::whereDate('created_at','<=',$request->from_date)->sum('paid_amount');
        }else{
