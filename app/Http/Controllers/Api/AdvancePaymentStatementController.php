...
class AdvancePaymentStatementController extends Controller
{
    ...
    
    {
        ...
        $paidby = $advancePaymentCollection1->map(function ($item) {
            $credit=AdvancePayment::where('received_by',$item->id)->where('payment_account_id',$request->id)->sum('amount'); 
            $debit=AdvancePayment::where('payment_account_id',$item->id)->where('received_by',$item->id)->sum('amount'); 
            $item['name']  =$item->name;
